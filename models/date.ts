...
export default class UtilDate {
    ...
    constructor(date: string | number | Date = new Date(), lng: suportedLng = 'es') {
        this.date = new Date(date)
        this.lng = new Lng(lng)
    }

    private addHours(h: number | string): UtilDate {
        return new UtilDate(this.date.setHours(this.date.getHours() + parseInt(`${h}`)));
    }
    ...
    private addMinutes(minutes: number | string): UtilDate {
        return new UtilDate(this.date.setMinutes(this.date.getMinutes() + parseInt(`${minutes}`)));
    }
    ...
    private addSeconds(seconds: number | string): UtilDate {
        return new UtilDate(this.date.setSeconds(this.date.getSeconds() + parseInt(`${seconds}`)));
    }
    ...
    private addMonths(month: number | string): UtilDate {
        return new UtilDate(this.date.setMonth(this.date.getMonth() + parseInt(`${month}`)));
    }

    private addDays(days: number | string): UtilDate {
        return new UtilDate(this.date.setDate(this.date.getDate() + parseInt(`${days}`)));
    }
    ...
    private addYears(years: number | string): UtilDate {
        return new UtilDate(this.date.setFullYear(this.date.getFullYear() + parseInt(`${years}`)));
    }
    ...
    startOfYear(): UtilDate {
        return new UtilDate(this.date).set(1, 'months')
    }

    add(amount: number | string, key: 'seconds' | 'minutes' | 'hours' | 'days' | 'months' | 'years'): UtilDate {
        switch (key) {
            case 'seconds': return this.addSeconds(amount);
            case 'minutes': return this.addMinutes(amount);
            case 'hours': return this.addHours(amount);
            case 'days': return this.addDays(amount);
            case 'months': return this.addMonths(amount);
            case 'years': return this.addYears(amount);
            default: return this;
        }
    }

    subtract(amount: number | string, key: 'seconds' | 'minutes' | 'hours' | 'days' | 'months' | 'years'): UtilDate {
        const value = parseInt(`${amount}`, 10)
        const negativeAmount = value >= 0 ? value * -1 : value
        switch (key) {
            case 'seconds': return this.addSeconds(negativeAmount);
            case 'minutes': return this.addMinutes(negativeAmount);
            case 'hours': return this.addHours(negativeAmount);
            case 'days': return this.addDays(negativeAmount);
            case 'months': return this.addMonths(negativeAmount);
            case 'years': return this.addYears(negativeAmount);
            default: return this;
        }
    }

    set(amount: number | string, key: 'seconds' | 'minutes' | 'hours' | 'days' | 'months' | 'years'): UtilDate {
        switch (key) {
            case 'seconds': return this.setSeconds(amount);
            case 'minutes': return this.setMinutes(amount);
            case 'hours': return this.setHours(amount);
            case 'days': return this.setDays(amount);
            case 'months': return this.setMonth(amount);
            case 'years': return this.setYears(amount);
            default: return this;
        }
    }
    ...
    format(format: string) {
        ...
        return format
            ...
            .replace(/month-mmm/, this.monthhortName)
    ...
    createDateArray(start: Date | string, end: Date | string): Array<Date> {
        ...
        while (localStart <= localEnd) {
            ...
            localStart = new Date(new Date(start).setDate(new Date(start).getDate() + 1))
        }
        return dates
    }
