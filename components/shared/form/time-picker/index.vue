        ...
        <div class="e-field__control" v-click-outside="handleOutsideMenu">
            <div class="e-field__slot" @mouseenter="handleHover(true)" @mouseleave="handleHover(false)">
                ...
                <div class="e-time-picker__fields">
                    <label :for="`${id}-hours`" :class="[textColor, 'e-label']" :style="labelStyle">
                        <slot name="label">{{ label }}</slot>
                    </label>
                    <input ref="hours" :id="`${id}-hours`" class="input--text" :value="hourLabel" data-hours readonly
                        limit="2" type="number" placeholder="00" autocomplete="off" max="24" min="0" @focus="handleFocus"
                        @blur="handleBlur" />
                    <div class="e-time-picker__separator">
                        <slot name="separator"><span class="mt-n1">:</span></slot>
                    </div>
                    <input ref="minutes" :id="`${id}-minutes`" class="input--text" step="15" data-minutes
...
export interface Props {
    arrowDown?: string; disabled?: boolean; dense?: boolean; readonly?: boolean; hoursStep?: number
...
const { fieldClass, dirty, focused, id, showDetails, textColor, color,
    ...
    handleClickAppendIcon } = useField()
